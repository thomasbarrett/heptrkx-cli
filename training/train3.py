...
def main():
    ...
    learning_rate           = 0.001
    restore_path            = './results/train3/models/model39.ckpt'
    restore_index           = 40
    ...
    node_params_truth = node_parameters(input_ph, target_ph, triplets_ph)
    node_params = node_parameters(input_ph, output_op.edges, triplets_ph)
    
    node_param_in = 10000 * tf.gather(node_params, input_ph.senders)
    node_param_out = 10000 * tf.gather(node_params, input_ph.receivers)
    node_param_avg = node_param_out + node_param_in * 0.5

    node_param_in_truth = 10000 * tf.gather(node_params_truth, input_ph.senders)
    node_param_out_truth = 10000 * tf.gather(node_params_truth, input_ph.receivers)
    node_param_avg_truth = node_param_out_truth + node_param_in_truth * 0.5
    edge_param_loss_op =  tf.losses.mean_squared_error(node_param_avg_truth, node_param_avg)
    binary_classifier_loss_op = tf.losses.log_loss(target_ph, tf.squeeze(output_op.edges))
    loss_op = edge_param_loss_op + binary_classifier_loss_op
            ...
            if output_time_lapse > log_every_seconds:
                ...
                for _, _, input_batch, labels, triplets in batch_iterator(base_dir, 1, test=True):
                    ...
                    train_values = sess.run({
                        ...
                        "edge_param_loss": edge_param_loss_op,
                        "loss": loss_op,
                        "target": target_ph,
                    ...
                    edge_param_loss.append(train_values['edge_param_loss'])
                ...
                precision_op = tf.metrics.precision_at_thresholds(batch_labels, batch_predictions,thresholds.tolist())    
                recall_op = tf.metrics.recall_at_thresholds(batch_labels, batch_predictions, thresholds.tolist())
                ...
                edge_param_loss = np.mean(np.array(edge_param_loss))

                # Print training progress information
                print('\repoch: {} progress: {:.4f} classifier loss: {:.4f} edge parameter loss: {:.4f} precision: {:.4f} recall: {:.4f} threshold: {:.4f}'.format(
                    iteration, batch_index/batch_count, classifier_loss, edge_param_loss,
